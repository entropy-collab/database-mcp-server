...
public class BackupTools extends McpToolBase {
    ...
    @McpTool(description = """
            ...
            前置条件：先调用 createNamedConnection 注册数据库连接。备份记录只存在服务端内存中（条数与天数上限见 getBackupConfig，默认 200 条 / 7 天），进程重启即全部丢失，过期或被挤出后无法再用于恢复，重要备份请及时取走 sqlStatements 自行落盘。
    ...
    public Map<String, Object> backupTable(
            ...
            @McpToolParam(description = "备份类型，取值 FULL 或 INCREMENTAL，省略时按 FULL 处理。INCREMENTAL 还需服务端开启 incrementalEnabled，且表上要有可识别的时间戳水位列（如 UPDATED_AT、UPDATE_TIME、CREATED_AT），否则会返回 error 而不是退化成全量",
        ...
        return safeExecute(() -> {
            ...
            Map<String, Object> result = "INCREMENTAL".equals(actualType) && backupProps.incrementalEnabled()
                    ? backupService.backupDataIncremental(tableName, maxRows, connection)
                    : backupService.backupData(tableName, maxRows, connection);
    ...
    @McpTool(description = """
            ...
            注意：结构备份记录的 type 是 SCHEMA，只能用 restoreBackup 回放 DDL；用它的 backupId 调 quickRestore 会被拒绝，因为 quickRestore 先清空表、而结构备份里没有任何 INSERT。备份记录只存在服务端内存中，进程重启即丢失，也会因条数或天数上限被静默淘汰（上限见 getBackupConfig），需要长期留存请自行保存返回的 ddl。
    ...
    public Map<String, Object> backupSchema(
        ...
        return safeExecute(() -> backupService.backupSchema(tableName, connection));
    }

    @McpTool(description = """
            【回放备份脚本恢复】在单个事务中回放备份记录里保存的全部 SQL 语句，把数据追加回目标库。
            前置条件：需要先有 backupId（用 listBackups 找、用 getBackup 确认）。备份记录只存在服务端内存中，进程重启或被淘汰后取不到，此时返回 NOT_FOUND 而不是空回放。恢复前不会清空目标表，因此已存在的主键会导致冲突报错；任一语句失败则整个事务回滚，表保持原样，备份记录不会被改动、可以重试。
    ...
    public Map<String, Object> restoreBackup(
            ...
            @McpToolParam(description = ToolParams.CONNECTION_DESCRIPTION, required = false) String connection) {
        return safeExecute(() -> backupService.restoreBackup(backupId, connection));
    }

    @McpTool(description = """
            【整表还原】先清空目标表，再回放备份中的 INSERT 语句，把表还原成备份时的状态。
            前置条件：需要先有 backupId（用 listBackups 找、用 getBackup 确认）。备份记录只存在服务端内存中，进程重启或被淘汰后取不到，此时返回 NOT_FOUND 且不动目标表。只接受「完整的数据备份」：type=SCHEMA 的结构备份、status=PARTIAL 的截断备份、以及不含任何 INSERT 的备份都会返回 error 并且不动目标表——否则清空之后无可回放内容，等于把表清空并提交。清空用的是 DELETE 而非 TRUNCATE，因此整个过程在一个事务内：失败会回滚，原有数据不丢；代价是超大表较慢。备份脚本中的非 INSERT 语句会被跳过。
    ...
    public Map<String, Object> quickRestore(
            ...
            @McpToolParam(description = ToolParams.CONNECTION_DESCRIPTION, required = false) String connection) {
        return safeExecute(() -> backupService.quickRestore(backupId, connection));
    }
    ...
    @McpTool(description = """
            ...
            注意：清单只反映服务端内存里现存的记录，进程重启后为空；超出条数或天数上限的记录已被静默淘汰，不会出现在这里。上限见 getBackupConfig。
    ...
    public Map<String, Object> listBackups(
        ...
        return safeExecute(() -> {
            ...
            List<BackupMetadata> records = metadataRepository.list(connection, tableName, lim);
    ...
    @McpTool(description = """
            ...
            前置条件：需要已知 backupId；不确定时先用 listBackups 检索。备份记录只存在服务端内存中，进程重启即丢失，也会因条数或天数上限被静默淘汰（上限见 getBackupConfig），找不到时返回 NOT_FOUND 错误，说明该备份已不可用于恢复。
    ...
    @McpTool(description = """
            ...
            使用场景：明确某条备份已作废、需要立即释放它占用的内存（备份脚本全文都在内存里，是本服务最容易吃内存的地方）。
    ...
    @McpTool(description = """
            ...
            注意：内存存储本身已按 entropy.mcp.database.backup.retention-days 硬过期，所以传入大于该值的天数不会清出任何东西；本工具的用途是按更短的天数提前清理。
            不要用于：删除指定的某一条备份（用 deleteBackup）。
            标签：[write, backup, cleanup, retention, destructive]
            """,
             annotations = @McpTool.McpAnnotations(destructiveHint = true, idempotentHint = true, openWorldHint = false))
    public Map<String, Object> cleanupBackups(
            @McpToolParam(description = "保留天数，创建时间早于「当前时间减该天数」的记录会被删除。省略时取服务端配置 entropy.mcp.database.backup.retention-days（默认 7）", required = false) Integer retentionDays) {
        return safeExecute(() -> {
    ...
    @McpTool(description = """
            【查看备份模块配置】读取服务端备份模块的当前配置、内存存储上限与备份留存数量。
            使用场景：调用备份工具前先确认功能是否开启、增量备份是否可用、单次备份的行数上限是多少、以及备份能在服务端留存多久。
            返回字段：enabled、incrementalEnabled、maxBackupRows、storageInMemoryOnly（恒为 true）、maxRecords（内存中最多保留的备份条数）、retentionDays（备份记录的硬过期天数）、totalBackups（当前留存的备份条数）。
            注意：备份记录只存在进程内存里，重启即全部丢失，与 retentionDays 无关；超过 maxRecords 或 retentionDays 的记录被静默淘汰，之后 restoreBackup / quickRestore 会报 NOT_FOUND。要长期留存请在 backupTable 返回时取走 sqlStatements 自行落盘。
    ...
    public Map<String, Object> getBackupConfig() {
        return safeExecute(() -> success(Map.of(
                ...
                // 存储形态必须回给调用方：模型看到 retentionDays 会默认备份是持久的
                "storageInMemoryOnly", true,
                "maxRecords", metadataRepository.maxRecords(),
                "retentionDays", metadataRepository.retentionDays(),
