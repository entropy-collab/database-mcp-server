...
public final class ValidationUtils {
    ...
    public static void validateIdentifier(String value, String paramName) {
        if (value == null || value.isBlank()) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, paramName + " cannot be blank");
        }
        if (!value.matches("[A-Za-z][A-Za-z0-9_$#]*")) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, paramName + " contains invalid characters: " + value);
        }
    }

    public static void validateHost(String host) {
        if (host == null || host.isBlank()) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "host cannot be blank");
        }
        if (!host.matches("[A-Za-z0-9._-]+")) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "host contains invalid characters: " + host);
        }
    }

    public static void validatePort(String port) {
        if (port == null || port.isBlank()) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "port cannot be blank");
        }
        try {
            int p = Integer.parseInt(port);
            if (p < 1 || p > 65535) {
                throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "port out of range: " + p);
            }
        } catch (NumberFormatException e) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "port must be a number: " + port);
        }
    }

    public static void validateServiceName(String serviceName) {
        if (serviceName == null || serviceName.isBlank()) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "serviceName cannot be blank");
        }
        if (!serviceName.matches("[A-Za-z0-9._]+")) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, "serviceName contains invalid characters: " + serviceName);
        }
    }
    ...
    public static void requireNotNull(Object value, String paramName) {
        if (value == null) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, paramName + " cannot be null");
        }
    }

    public static void requireNotEmpty(Collection<?> collection, String paramName) {
        if (collection == null || collection.isEmpty()) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, paramName + " cannot be empty");
        }
    }

    public static void requirePositive(int value, String paramName) {
        if (value <= 0) {
            throw new DatabaseMcpException(ErrorCode.PARAMETER_VALIDATION_FAILED, paramName + " must be positive");
        }
    }
