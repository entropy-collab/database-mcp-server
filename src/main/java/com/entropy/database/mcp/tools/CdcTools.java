...
public class CdcTools extends McpToolBase {
    ...
    @McpTool(description = """
            ...
            前置条件：必须传 connection——先调用 createNamedConnection 注册连接。
            使用场景：接入变更同步前的可行性判断；CDC 工具报错后回头确认是能力缺失还是参数问题。
            检测方式：Oracle 检查 recyclebin 与闪回日志是否开启；MySQL 检查 log_bin / binlog_format 是否配置；PostgreSQL 检查 pglogical 扩展、逻辑订阅或发布是否存在；H2、SQL Server、SQLite、DB2 等方言未实现 CDC，一律返回 false。
    ...
    @McpTool(description = """
            ...
            前置条件：connection 与 table 必填（connection 虽标为可选，为空会被拒绝）；建议先用 checkCdcSupport 确认能力、再用 getCurrentLsn 拿到起始位点。各方言差异很大：Oracle 走闪回版本查询（VERSIONS BETWEEN SCN），能给出真实的 I/U/D 操作类型，但 beforeJson/afterJson 恒为空、primaryKeys 是 ROWID；MySQL 与 PostgreSQL 都读约定的触发器审计表「原表名_audit」（需事先自行创建并挂触发器，MySQL 的 binlog 只能靠 Debezium 消费、这里读不到），因此操作类型无法还原，changeType 恒为 TRIGGER_AUDIT；H2、SQL Server、SQLite、DB2 未实现，调用直接报错。
    ...
    public Map<String, Object> readChanges(
            ...
            @McpToolParam(description = "起始位点：Oracle 传 SCN、PostgreSQL 传 WAL LSN 的整数形式、MySQL 传由 binlog 文件序号与偏移打包成的整数。传 null 或非正数时会自动取当前最新位点作为起点（因此通常读不到历史变更），做增量拉取时应传上一次的位点") Long fromLsn) {
        return safeExecute(() -> {
    ...
    @McpTool(description = """
            ...
            前置条件：connection 必填（虽标为可选，为空会被拒绝）。Oracle 读 v$database.CURRENT_SCN（需要 v$ 视图权限）；MySQL 执行 SHOW MASTER STATUS（需要 REPLICATION CLIENT 权限，返回的 binlog 文件名与偏移会被打包成一个递增整数）；PostgreSQL 读 pg_current_wal_lsn() 并把十六进制 LSN 解码为整数；H2、SQL Server、SQLite、DB2 未实现，调用直接报错。
    ...
    @McpTool(description = """
            ...
            前置条件：属于 DDL 操作，服务端必须同时开启 entropy.mcp.database.ddl.allowed=true 与 entropy.mcp.database.cdc.enable-mirror-tables=true，否则直接拒绝；connection、sourceTable、targetSchema、targetTable 必填；所有库表名都必须是合法标识符（不接受引号或特殊字符）。目标表若已存在会因建表失败而报错。仅 Oracle、MySQL、PostgreSQL 实现，其他方言报「不支持」。
    ...
    @McpTool(description = """
            ...
            前置条件：connection 与 name 必填（connection 虽标为可选，为空会被拒绝）；建议先用 checkCdcSupport 确认该库支持 CDC。订阅只保存在服务进程内存中，重启即丢失；同名订阅会被直接覆盖。
    ...
    @McpTool(description = """
            ...
            前置条件：建议先用 listSubscriptions 确认订阅名。传入不存在的名字不会报错。
            使用场景：不再关注某个表范围时清理订阅。
    ...
    @McpTool(description = """
            ...
            前置条件：connection 必填（虽标为可选，为空会被拒绝）。事件计数与最后事件时间只统计本进程通过 readChanges 实际读到的量，进程重启后归零。
    ...
    @McpTool(description = """
            ...
            使用场景：调用 createMirrorTable 等受开关约束的工具前，先确认功能是否开启、默认轮询间隔是多少。
